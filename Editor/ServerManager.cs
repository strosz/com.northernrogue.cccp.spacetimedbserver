...
{       
    ...
    public async Task CheckDockerImageTag(bool manualCheck) // Only runs in Docker once when Docker has started
    {
        ...
        bool updateAvailable = result.updateAvailable;
        bool hasLatestTagWithVersions = result.hasLatestTagWithVersions;
        ...
        {
            // Only show this dialog once per editor session
            if (!SessionState.GetBool("LatestTagRemovalPromptShown", false))
            {
                SessionState.SetBool("LatestTagRemovalPromptShown", true);
